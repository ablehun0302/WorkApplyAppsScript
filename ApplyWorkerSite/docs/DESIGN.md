...
### 2026-09-04(3): 근무자 신청 화면에서 오늘 이전 날짜 수정 차단

근로자가 "신청/수정" 화면(현재 2주 창)에서 이미 지난 날짜의 주간/야간 버튼도 자유롭게 눌러 과거 근무 신청을 바꿀 수 있었다. `index.html`의 `buildDateList()`에서 각 날짜를 오늘(`formatDate(new Date())`)과 비교해, 오늘 이전 날짜의 버튼은 `onclick`을 붙이지 않고 회색으로 표시(`.shift-btn.disabled`)해 클릭이 되지 않게 했다. 다만 클라이언트 우회로 `saveRecord`가 과거 날짜 값을 포함해 직접 호출될 수 있으므로, 서버(`WorkSystemSheet.gs`)에서도 오늘 이전 날짜분은 클라이언트가 보낸 값을 무시하고 기존 `Data` 시트 값을 그대로 유지하도록 병합 로직을 추가했다(`removeCanceledAssignments_`도 이 병합된 값 기준으로 취소분을 계산). 관리자 화면의 "신청 전체 취소"(`deleteRecord`)는 이번 변경 범위에 포함하지 않았다 — 그건 근로자가 개별 날짜를 "바꾸는" 것이 아니라 신청 자체를 통째로 취소하는 별개 동작이라 판단했다.

### 2026-09-04(2): 전체신청자 목록에 근무지 필터 추가

"전체신청자 목록"이 이름 오름차순으로만 나열되어, 최근 특정 근무지에서 일한 사람만 골라보기 어려웠다. `getAssignments`가 이미 반환하는 배치별 `date`/`key`/`location`(위 K열 근무지 커밋 참고)을 재사용해, 목록 상단에 근무지 드롭다운(전체+`LOCATIONS`)과 기간 드롭다운(1/3/6/12개월, 기본 3개월)을 추가했다. `loadRoster()`가 `getAssignments`도 함께 불러와 원본 데이터를 전역에 캐시해두고, `renderRoster()`가 인자 없이 캐시와 두 드롭다운 값을 읽어 "최근 근무지(N개월)" 컬럼을 계산·표시하며, 근무지 필터가 선택되면 해당 근무지가 최근 근무지 목록에 포함된 사람만 남긴다. 드롭다운을 바꿔도 서버 재호출 없이 캐시된 데이터로 즉시 재계산된다. 근무지 컬럼은 오늘 추가된 기능이라 그 이전 배치 기록은 근무지가 비어 있어 필터에 걸리지 않는다.

### 2026-09-04: 근무자 신청 취소(전체/부분) 시 취소된 (날짜,시프트)만 Assign/History에서 제거

기존 `deleteRecord`(근무자 본인의 "신청 전체 취소")는 `Data` 행 삭제와 함께 해당 근로자의 `Assign` 배치 행을 키(key) 기준으로 전부 삭제해, 이번 취소와 무관한 과거 배치 이력까지 사라지는 문제가 있었다. 또한 근로자가 신청 수정 화면에서 일부 날짜만 체크 해제하는 "부분취소"의 경우 `saveRecord`가 `Assign`을 전혀 건드리지 않아, 이미 취소된 날짜의 배치가 그대로 남아있는 문제도 있었다.

공용 함수 `removeCanceledAssignments_(key, oldShifts, newShifts)`를 추가해 두 경우를 모두 처리한다. 저장/삭제 직전의 `shiftsJSON`(oldShifts)과 새로 저장할 값(newShifts, 전체 취소 시 빈 배열)을 (날짜,시프트) 단위로 비교해, 이번에 새로 빠진 조합에 대해서만 `Assign`에서 해당 배치를 제거하고, 같은 날짜의 다른 시프트 배치가 남아있지 않을 때만 `removeHistory_`로 `History`(월별 근로일수 집계용) 기록도 제거한다. `saveRecord`/`deleteRecord` 모두 이 함수를 호출하도록 수정했다.

주의: 근로자 신청 화면(`getTwoWeekDates`)은 "이번 주~다음 주" 2주 창만 렌더링하므로, 주가 바뀌어 화면에서 사라진 지난주 날짜는 재저장 시 `shifts`에 아예 들어오지 않는다. 이를 구분 없이 처리하면 단순히 주가 넘어가 화면 밖으로 밀려난 지난주 배치까지 "취소"로 오인해 지워버리는 문제가 생긴다. 이를 막기 위해 `getCurrentTwoWeekDateSet_()`로 서버에서도 동일한 2주 창을 계산해, oldShifts 중 이 창 밖의 날짜는 취소 판정에서 제외하도록 `removeCanceledAssignments_`를 수정했다.

### 2026-09-03(4): 광고 수신 동의 여부 저장 및 발송 시 반영

`Data` 시트에 `adConsent`(L열, `'Y'`/`'N'`) 컬럼을 추가했다. 신청서 화면에 "광고성 정보 수신에 동의합니다(선택)" 체크박스를 추가해 `saveRecord`가 저장하고, 재조회 시 `lookupRecord`가 값을 돌려줘 체크 상태를 복원한다. `batchSaveRecords`(관리자 직접 추가)는 동의 항목이 없으므로 기존 값을 보존하고 신규는 빈값(미동의 취급)으로 둔다. 관리자 "전체신청자 목록"에 "광고동의" 컬럼을 표시하고, "광고성 메시지" 체크 후 발송 시 미동의자는 개별 발송은 확인창으로 차단, 일괄 발송은 대상에서 자동 제외(제외 인원 수 안내)하도록 `index.html`을 수정했다.
